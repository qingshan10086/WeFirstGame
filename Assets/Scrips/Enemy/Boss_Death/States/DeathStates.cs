...
public class GroundedState : EnemyState
{
    ...
    public override void Updata()
    {
        ...
        // 优先检查是否有施法条件（距离远且概率通过且冷却）
        if (enemy.CanStartCast())
        {
...
public class IdleState : GroundedState
{
    ...
    public override void Enter()
    {
        ...
        stateTimer = enemy.idleTime;//初始化站立时间
    }
    public override void Updata()
    {
        base.Updata();

        if (stateTimer < 0)//站立时间到即进入移动状态
        {
...
public class BattleState : GroundedState
{
    ...
    public override void Updata()
    {
        ...
        // 检查血量触发召唤（优先），满足则进入 SummonState
        if (enemy.CanStartSummon())
        {
            stateMachine.ChangeState(enemy.summonState);
            return;
        }
        if (enemy.CanStartControl())
        {
            stateMachine.ChangeState(enemy.controlState);
            return;
        }
        ...
        if (enemy.IsPlayerDetected())//如果检测到玩家
        {
            ...
            float dist = enemy.IsPlayerDetected().distance;
            if (dist < 0) dist = Vector2.Distance(player.position, enemy.transform.position);

            if (dist < enemy.attackDistance)//如果距离足够，则进入攻击状态
            {
                if (CanAttack())
                {
...
public class AttackState : EnemyState
{
    ...
    public override void Exit()
    {
        base.Exit();
        enemy.lastTimeAttacked = Time.time;
        // 普通攻击执行完，记录一次普通攻击（用于触发强化普攻）
        enemy.RegisterNormalAttack();
    }
    public override void Updata()
    {
        base.Updata();
