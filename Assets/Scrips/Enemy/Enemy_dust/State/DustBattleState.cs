using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class DustBattleState : EnemyState
{
    private Transform player;//申明玩家位置信息
    private Enemy_Dust enemy;//申明怪物
    private int moveDir;//申明移动方向

    private bool isFirstAttack = true;//是否是第一次攻击
    public DustBattleState(Enemy _enemyBase, EnemyStateMachine _stateMachine, string _animBoolName, Enemy_Dust _enemy) : base(_enemyBase, _stateMachine, _animBoolName)
    {
        this.enemy = _enemy;
    }
    public override void Enter()
    {
        base.Enter();

        player = PlayerManager.instance.player.transform;//获取玩家位置信息

        
    }

    public override void Exit()
    {
        base.Exit();
    }

    public override void Updata()
    {
        base.Updata();

        if (enemy.IsPlayerDetected())//如果检测到玩家
        {
            stateTimer = enemy.battleTime;//重置战斗状态持续时间

            if (enemy.IsPlayerDetected().distance < enemy.attackDistance)//如果距离足够，则进入攻击状态
            {
                if (CanAttack())
                {
                    // 第一次攻击前保持不动：归零速度并标记已触发第一次攻击
                    if (isFirstAttack)
                    {
                        enemy.ZeroVelocity();
                        isFirstAttack = false;

                        enemy.StartCoroutine(ShowBar());
                    }
        
                    stateMachine.ChangeState(enemy.attackState);
                    return; // 切换状态后立即返回，避免继续执行本帧的移动逻辑
                }
            }

        }
        else
        {
            if (stateTimer < 0 || Vector2.Distance(player.transform.position, enemy.transform.position) > 20)//如果距离过远，进入站立状态
            {
                stateMachine.ChangeState(enemy.idleState);
                return; // 切换状态后立即返回
            }
        }

        if (player.position.x > enemy.transform.position.x)//战斗状态始终面向玩家
        {
            moveDir = 1;
        }
        else if (player.position.x < enemy.transform.position.x)
        {
            moveDir = -1;
        }

        if (isFirstAttack)
        {
            // 第一次攻击未发生前保持不动（已经在触发攻击时归零） 
            enemy.HideHealthBar();
        }
        else
        {
            enemy.SetVelocity(enemy.moveSpeed * moveDir, rb.velocity.y);//战斗速度
        }
    }

    private IEnumerator ShowBar()
    {
        yield return new WaitForSeconds(2.0f);
        enemy.ShowHealthBar();
    }

    private bool CanAttack()//攻击冷却设置
    {
        if (Time.time >= enemy.lastTimeAttacked + enemy.attackCooldown)
        {
            enemy.lastTimeAttacked = Time.time;
            return true;
        }
        else
        {
            return false;
        }
    }
}
